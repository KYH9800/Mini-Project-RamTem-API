...
describe('Layered Architecture Pattern Posts Controller Unit Test', () => {
  beforeEach(() => {
    jest.resetAllMocks(); // 각 test가 실행되기 전에 실행됩니다. 모든 Mock을 초기화합니다.
    // mockResponse.status의 경우 메서드 체이닝으로 인해 반환값이
    // Response(자신: this)로 설정되어야합니다.
    mockResponse.status = jest.fn(() => {
      return mockResponse;
    });
  });

  test('case: PostsController getPost Method by Success', async () => {
    ...
    // 3. findAllPost Method에서 반환된 posts 변수의 값을 res.json Method를 이용해
    // { data: posts }의 형식으로 반환합니다.
    expect(mockResponse.json).toHaveBeenCalledWith({
      data: postsReturnValue,
    });
  });

  test('case: PostsController createPost Method by Success', async () => {
  ...
  test('case: PostsController createPost Method by Invalid Params Error', async () => {
